...
function fGetGoogleObject(result, userContext) {
    ...
    if (!result.IsPostback) {
        ...
        if (result.RecenterMap) {
            map.setCenter(new google.maps.LatLng(result.CenterPoint.Latitude, result.CenterPoint.Longitude));
            GoogleMapControl.GService.RecenterMapComplete();
        }
    ...
    for (i = 0; i < result.Points.length; i++) {
        ...
        if (marker == null) {
            ...
            google.maps.event.addListener(markers.getLastValue(), "dragend", function () { GoogleMapControl.GService.SetLatLon(this.value, this.position.lat(), this.position.lng()); RaiseEvent('PushpinDrag', this.value); });
...
function DrawGoogleMap() {
    ...
    GoogleMapControl.GService.GetGoogleObject(fGetGoogleObject);
}
